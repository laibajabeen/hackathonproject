...
def get_session(sid: str) -> SQLiteSession:
    """Reuse per-session-id to avoid re-opening db."""
    s = _SESSIONS.get(sid)
    if s is None:
...
def user_output(info: Query_Output):
    """
    Function tool that formats the output of the real estate agent query.
    """
    return (
        f"Following is the property found matching your search, "
        f"Price: {info.price}, Location: {info.location}, Postcode: {info.postcode}, "
...
studio_agent = Agent(
    ...
    instructions="""You are a studio agent assistant. You help users find information about studios in the UK from websites like Rightmove, Zoopla, and SpareRoom.
    You answer questions related to price, location, postcode, link, and property type, using the web search tool to get live data every time.
    You also have memory capabilities!""",
    tools=[WebSearchTool()],
    model=MODEL,
...
location_agent = Agent(
    ...
    instructions="""You are a location agent assistant. You help users find information about locations in the UK using the geo_tool to convert location names into coordinates (latitude and longitude). 
    You provide accurate and concise information based on user queries.
    You also have memory capabilities!""",
    tools=[WebSearchTool()],
    model=MODEL,
)
